...
function WhoAmI() {
  const { displayed, done, cursorChar } = useTypewriter('>_ $ whoami', { speed: 55, delay: 300 });

  return (
    <section id="whoami" style={{ minHeight: '100vh', width: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '120px 40px 80px' }}>
      <div style={{ width: '100%', maxWidth: '1560px', marginBottom: '48px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <span style={{ color: 'var(--accent)', fontFamily: 'var(--font-mono)', fontSize: '18px' }}>
          {displayed}
          {!done && <span style={{ opacity: 0.8, animation: 'blink-cur 1s step-end infinite' }}>{cursorChar}</span>}
        </span>
        ...
        <div style={{ width: '440px', flexShrink: 0 }}>
          <TerminalCard title="profile.json" accentBorder>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '32px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '15px' }}><span style={{ color: 'var(--text-muted)' }}>📍 location: </span><span style={{ color: 'var(--accent)' }}>"{profile.location}"</span></div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '15px' }}><span style={{ color: 'var(--text-muted)' }}>⚡ status: </span><span style={{ color: 'var(--green)' }}>"{profile.status}"</span></div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '15px' }}><span style={{ color: 'var(--text-muted)' }}>&lt;/&gt; focus: </span><span style={{ color: 'var(--text-primary)' }}>"web &amp; mobile"</span></div>
            </div>
